...
describe('decompPrime', () => {
  ...
  it('should be able to decompose a product of two numbers', async () => {
    await fc.assert(
  ...
  it('should compute the same factors as to the concatenation of the one of a and b for a times b', async () => {
    await fc.assert(
      fc.property(fc.integer({ min: 2, max: MAX_INPUT }), fc.integer({ min: 2, max: MAX_INPUT }), (a, b) => {
