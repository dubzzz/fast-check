...
describe('entityGraph (integration)', () => {
  ...
  describe('organization with initialPoolConstraints and unicityConstraints', () => {
    type Extra = {};
    const extraParameters: fc.Arbitrary<Extra> = fc.record({});

    const entityGraphBuilder = (_extra: Extra) => {
      return entityGraph(
        {
          employee: { firstName: string(), lastName: string() },
          team: { name: string() },
        ...
        },
        {
          ...
          unicityConstraints: {
            employee: (value) => `${value.firstName} ${value.lastName}`,
    ...
    const isCorrect = (value: UnArbitrary<ReturnType<typeof entityGraphBuilder>>, _extra: Extra) => {
      const allEmployees = new Set(value.employee);
      const allTeams = new Set(value.team);
      // Checking basic structure
      expect(value).toStrictEqual({
      ...
      // Checking pool constraints
      expect(value.employee.length).toBeGreaterThanOrEqual(2);
      expect(value.team.length).toBeGreaterThanOrEqual(1);
      // Checking employees
      for (const employee of value.employee) {
        expect(employee).toStrictEqual({
          firstName: expect.any(String),
