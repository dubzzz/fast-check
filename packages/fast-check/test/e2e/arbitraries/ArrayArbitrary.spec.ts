...
function biasIts<T>(label: string, arb: fc.Arbitrary<T>) {
  it(`Should be biased by default and suggest extreme entries more often [${label}]`, async () => {
  ...
  it(`Should not be able to find the issue if unbiased (barely impossible) [${label}]`, async () => {
    // Falsy implementation of removeDuplicates
    const removeDuplicates = (arr: T[]) => [...arr];
    // Expect a failure
    const out = await fc.check(
