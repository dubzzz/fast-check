...
/**
 * Defines the shape of a single entity type by mapping each field name to an arbitrary for that field.
 *
 * This type represents the structure of one entity type, where each field is associated with
 ...
 * of each entity type. Each key represents an entity type name, and its value defines the
 * arbitraries for that entity's data fields.
 *
 * @example
 * ```typescript
 * {
 ...
 * - `'successor'`: Target must appear later in the entity list (prevents cycles when types match, otherwise behaves as 'any')
 *
 * @defaultValue 'any'
 * @remarks Since 4.5.0
 * @public
...
export type Relationship<TTypeNames> = {
   ...
   * - `'successor'`: Target must appear after the source in the entity array - prevents self-references and cycles when source and target types match (falls back to 'any' for different types)
   *
   * @defaultValue 'any'
   * @remarks Since 4.5.0
   */
 ...
 *   },
 *   team: {
 *     members: { arity: 'many', type: 'employee' }
 *   }
 * }
 * ```
 *
 ...
 * @example
 * ```typescript
 * // For entityGraph({ employee: {...} }, { employee: { team: {...} } })
 * // Produces type: { employee: Array<{ ...dataFields, team: TeamEntity }> }
 * ```
 *
 * @remarks Since 4.5.0
 * @public
