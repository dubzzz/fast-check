    ...
    return (
      this.lengthArb
        ...
        .map((lengthValue): [Value<T>[], unknown, number] => {
          ...
          return [
            value.slice(sliceStart).map((v, index) => new Value(cloneIfNeeded(v), safeContext.itemsContexts[index + sliceStart])), // array of length lengthValue.value
