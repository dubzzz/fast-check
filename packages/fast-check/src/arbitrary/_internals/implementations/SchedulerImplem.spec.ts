...
import type { ScheduledTask, TaskSelector } from './SchedulerImplem.js';
import { SchedulerImplem, numTicksBeforeScheduling } from './SchedulerImplem.js';
    ...
    it('should end whenever possible while never launching multiple tasks at the same time', async () => {
      ...
      await fc.assert(
        fc.asyncProperty(
          ...
          fc.iterator(fc.boolean(), { minLength: Number.POSITIVE_INFINITY }),
          ...
          async (
            ...
            const taskSelector: TaskSelector<unknown> = {
              ...
              nextTaskIndex: (scheduledTasks) => {
                const selectedId = nextTaskIndexSeed.next().value! % scheduledTasks.length; // seeds never end: minLength is +infinity
