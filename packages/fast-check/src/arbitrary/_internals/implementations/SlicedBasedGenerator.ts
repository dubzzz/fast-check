...
export class SlicedBasedGenerator<T> implements SlicedGenerator<T> {
  ...
  attemptExact(targetLength: number): void {
    if (targetLength !== 0 && nextInt(this.mrng, 1, this.biasFactor) === 1) {
      ...
      this.activeSliceIndex = eligibleIndices[nextInt(this.mrng, 0, eligibleIndices.length - 1)];
  ...
  next(): Value<T> {
    ...
    if (nextInt(this.mrng, 1, this.biasFactor) !== 1) {
      // We don't use the slices
      return this.arb.generate(this.mrng, this.biasFactor);
    }
    // We update the active slice
    this.activeSliceIndex = nextInt(this.mrng, 0, this.slices.length - 1);
    ...
    const rangeBoundaryA = nextInt(this.mrng, 0, slice.length - 1);
    const rangeBoundaryB = nextInt(this.mrng, 0, slice.length - 1);
