...
function fromSyncCached<Ts>(
  cachedValue: Awaited<ReturnType<IRawProperty<Ts>['run']>>,
): Awaited<ReturnType<IRawProperty<Ts>['run']>> {
  return cachedValue === null ? new PreconditionFailure() : cachedValue;
}

/** @internal */
function fromCached<Ts>(cachedValue: ReturnType<IRawProperty<Ts>['run']>): ReturnType<IRawProperty<Ts>['run']> {
  // check if really async
