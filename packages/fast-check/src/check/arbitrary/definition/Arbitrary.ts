...
/**
 * Assertion function provided to unmapper callbacks of {@link Arbitrary.map}.
 * Throws if the condition is falsy, narrowing the type in TypeScript.
 *
 * @remarks Since 4.7.0
 * @public
 */
export type UnmapGuard = (condition: boolean, message?: string) => asserts condition;

/** @internal */
function unmapGuard(condition: boolean, message?: string): asserts condition {
  if (!condition) {
    throw new Error(message ?? 'Value rejected by unmapper guard');
  }
}
...
export abstract class Arbitrary<T> {
   ...
   * @param unmapper - Optional unmap function, it will never be used except when shrinking user defined values. Must throw if value is not compatible (since 3.0.0). A `guard` helper is provided as second argument to simplify type-narrowing assertions (since 4.7.0)
   * @returns New arbitrary with mapped elements
   *
   * @remarks Since 0.0.1
   */
  map<U>(mapper: (t: T) => U, unmapper?: (possiblyU: unknown, guard: UnmapGuard) => T): Arbitrary<U> {
    // oxlint-disable-next-line no-use-before-define
...
class MapArbitrary<T, U> extends Arbitrary<U> {
  ...
  constructor(
    ...
    readonly unmapper?: (possiblyU: unknown, guard: UnmapGuard) => T,
  ) {
    super();
  ...
  canShrinkWithoutContext(value: unknown): value is U {
    if (this.unmapper !== undefined) {
      try {
        const unmapped = this.unmapper(value, unmapGuard);
