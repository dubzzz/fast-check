...
async function clearAllWorkersFor(property: Property<unknown>): Promise<void> {
 ...
 * @public
 */
export async function assert<Ts>(property: Property<Ts>, params?: Parameters<Ts>): Promise<void> {
  if (isMainThread) {
    // Main thread code
    try {
...
function workerProperty<Ts extends [unknown, ...unknown[]]>(
  ...
  if (isMainThread) {
    ...
    const property: Property<Ts> = fcProperty(...arbitraries, () => true);
