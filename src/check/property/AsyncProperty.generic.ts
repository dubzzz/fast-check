...
import { InterceptedError, IRawProperty, runIdToFrequency } from './IRawProperty';
...
export class AsyncProperty<Ts> implements IAsyncPropertyWithHooks<Ts> {
  ...
  async run(v: Ts): Promise<PreconditionFailure | InterceptedError | null> {
    ...
    try {
      ...
      // exception as InterceptedError in case of real failure
