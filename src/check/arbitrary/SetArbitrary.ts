...
function set<T>(arb: Arbitrary<T>, minLength: number, maxLength: number, compare: (a:T,b:T) => boolean): Arbitrary<T[]>;
function set<T>(arb: Arbitrary<T>,
        aLength?: number | ((a:T,b:T) => boolean),
        bLength?: number | ((a:T,b:T) => boolean),
        compareFn?: (a:T,b:T) => boolean): Arbitrary<T[]> {
    const minLength: number = bLength == null || typeof bLength !== 'number'
            ? 0 : aLength as number;
    const maxLength: number = aLength == null || typeof aLength !== 'number'
            ? 10 : (typeof bLength === 'number' ? bLength as number : aLength);
