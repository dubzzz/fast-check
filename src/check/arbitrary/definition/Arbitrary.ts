...
export default abstract class Arbitrary<T> {
   ...
   * @example
   * ```typescript
   * const arrayAndLimitArbitrary = fc.nat().flatMap((c: number) => fc.tuple( fc.array(fc.nat(c)), fc.constant(c)));
   * ```
   *
   * @param fmapper FlatMap function, to produce a new Arbitrary using a value from another Arbitrary
  ...
  flatMap<U>(fmapper: (t: T) => Arbitrary<U>): Arbitrary<U> {
    ...
    return new class extends Arbitrary<U> {
      generate(mrng: Random): Shrinkable<U> {
        const base = arb.generate(mrng);
        const mappedArb = fmapper(base.value);
        return mappedArb.generate(mrng);
      }
      withBias(freq: number): Arbitrary<U> {
