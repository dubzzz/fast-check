...
export default abstract class Arbitrary<T> {
   ...
   * @example
   * ```typescript
   * const arrayAndLimitArbitrary = fc.nat().then((c: number) => fc.tuple( fc.array(fc.nat(c)), fc.constant(c)));
   * ```
   *
   ...
   */
  then<U>(fmapper: (t: T) => Arbitrary<U>): Arbitrary<U> {
    const arb = this;
    return new class extends Arbitrary<U> {
      generate(mrng: Random): Shrinkable<U> {
      ...
      withBias(freq: number): Arbitrary<U> {
        return arb.withBias(freq).then(fmapper);
      }
    }();
  }
