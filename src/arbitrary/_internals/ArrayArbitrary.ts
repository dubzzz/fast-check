...
export class ArrayArbitrary<T> extends Arbitrary<T[]> {
  ...
  readonly preFilter: (tab: Value<T>[]) => Value<T>[];
  ...
  constructor(
    ...
    super();
    this.lengthArb = integer({ min: minLength, max: maxLength });
    this.preFilter = this.isEqual !== undefined ? buildCompareFilter(this.isEqual) : (tab: Value<T>[]) => tab;
  }
  ...
  private generateNItemsNoDuplicates(N: number, mrng: Random, biasFactorItems: number | undefined): Value<T>[] {
