...
/** @internal */
const numberLikeRegex = /^[0-9]+|0[xX][a-fA-F0-9]$/;
...
export function domain(constraints: DomainConstraints = {}): Arbitrary<string> {
  ...
  const publicSuffixArb = subdomainLabel(resolvedSizeMinusOne)
    // Chrome seems to reject (new URL) any domain ending by something that looks like a number
    // Following domains gets rejected: a.9, a.0x, a.0x9
    // But those ones are accepted: 9.a, 0x.a, 0x9.a
    // Or IP-likes: 999.10 but we don't want them to be generated by domain
    .filter((suffix) => !numberLikeRegex.test(suffix));
  return convertFromNext(
