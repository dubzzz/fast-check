...
export function sparseArray<T>(arb: Arbitrary<T>, constraints: SparseArrayConstraints = {}): Arbitrary<T[]> {
  ...
  const sparseArrayNoTrailingHole = set(tuple(nat(maxIndexAuthorized), arb), {
  ...
  return tuple(sparseArrayNoTrailingHole, integer({ min: minNumElements, max: maxLength })).map(
    (data) => {
      const sparse = data[0];
