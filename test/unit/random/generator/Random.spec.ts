...
describe('Random', () => {
    describe('next', () => {
        it('Should produce values within 0 and 2 ** n - 1', () => fc.assert(
    ...
    describe('nextInt', () => {
        it('Should produce values within the range', () => fc.assert(
        ...
        it('Should produce the same sequences given same seeds', () => fc.assert(
            fc.property(fc.integer(), fc.nat(MAX_SIZE), (seed, num) => {
                ...
                for (let idx = 0 ; idx != num ; ++idx)
                    if (mrng1.nextInt() != mrng2.nextInt())
                        return false;
                return true;
            })
        ));
    ...
    describe('nextDouble', () => {
        it('Should produce values within 0 and 1', () => fc.assert(
            fc.property(fc.integer(), fc.nat(MAX_SIZE), (seed, num) => {
                const mrng = new Random(prand.mersenne(seed));
                for (let idx = 0 ; idx != num ; ++idx) {
    ...
    describe('clone', () => {
        it('Should produce the same sequences', () => fc.assert(
            fc.property(fc.integer(), fc.nat(MAX_SIZE), (seed, num) => {
                const mrng1 = new Random(prand.mersenne(seed));
