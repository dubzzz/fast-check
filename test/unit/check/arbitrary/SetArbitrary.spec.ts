...
describe('SetArbitrary', () => {
    ...
    describe('set', () => {
        ...
        it('Should not contain twice the same value', () => fc.assert(
            fc.property(fc.integer(), (seed) => {
                const mrng = stubRng.mutable.fastincrease(seed);
        ...
        it('Should not contain twice the same value given a custom compare', () => fc.assert(
            fc.property(fc.integer(), (seed) => {
                const mrng = stubRng.mutable.fastincrease(seed);
        ...
        it('Should shrink values as valid sets', () => fc.assert(
            fc.property(fc.integer(), (seed) => {
                const mrng = stubRng.mutable.fastincrease(seed);
                const arb = set(char());
                const shrinkable = arb.generate(mrng);
                for (const s of shrinkable.shrink())
                    assert.ok(validSet(s.value), `${JSON.stringify(s.value)} is not a valid shrink for ${JSON.stringify(shrinkable.value)}`);
            })
        ));
