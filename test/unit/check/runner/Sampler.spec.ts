...
describe('Sampler', () => {
    describe('sample', () => {
        it('Should produce the same sequence given the same seed', () => fc.assert(
        ...
        it('Should produce the same sequence given the same seed and different lengths', () => fc.assert(
        ...
        it('Should produce exactly the number of outputs', () => fc.assert(
        ...
        it('Should produce exactly the number of outputs when called with number', () => fc.assert(
            fc.property(fc.nat(MAX_NUM_RUNS), fc.integer(), (num, start) => {
                const arb = stubArb.counter(start);
                const out = sample(arb, num);
                assert.equal(out.length, num, 'Should produce the right number of values');
    ...
    describe('statistics', () => {
        ...
        it('Should start log lines with labels', () => fc.assert(
            fc.property(fc.integer(), (seed) => {
                let logs: string[] = [];
                ...
                for (const l of logs) {
                    ...
                    assert.notStrictEqual(/^my_label_(\d+)!$/.exec(m[1]), null, `Label should have the format given by classifier: my_label_...!`);
                }
            })
        ));
        it('Should end log lines with percentage', () => fc.assert(
            fc.property(fc.integer(), (seed) => {
                let logs: string[] = [];
                const classify = (g: number) => g.toString();
                statistics(customGen(), classify, {seed: seed, logger: (v: string) => logs.push(v)});
        ...
        it('Should sum to 100% when provided a single classifier', () => fc.assert(
            fc.property(fc.integer(), (seed) => {
                let logs: string[] = [];
                const classify = (g: number) => g.toString();
                statistics(customGen(), classify, {seed: seed, logger: (v: string) => logs.push(v)});
        ...
        it('Should order percentages from the highest to the lowest', () => fc.assert(
            fc.property(fc.integer(), (seed) => {
                let logs: string[] = [];
                const classify = (g: number) => g.toString();
                statistics(customGen(), classify, {seed: seed, logger: (v: string) => logs.push(v)});
        ...
        it('Should be able to handle multiple classifiers', () => fc.assert(
            fc.property(fc.integer(), (seed) => {
                let logs: string[] = [];
                ...
                const associatedWithA = logs.filter(l => l.startsWith("a::")).map(l => l.slice(1)).sort();
                const associatedWithB = logs.filter(l => l.startsWith("b::")).map(l => l.slice(1)).sort();
                assert.deepEqual(associatedWithB, associatedWithA, "Should have the same logs for a:: and b::");
            })
        ));
        it('Should not produce more logs than the number of classified values', () => fc.assert(
