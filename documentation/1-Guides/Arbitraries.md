...
- `fc.frequency<T>(...warbs: WeightedArbitrary<T>[]): Arbitrary<T>` randomly chooses an arbitrary at each new generation. Should be provided with at least one arbitrary. The Probability to select a specific arbitrary is based on its weight, the higher it is, the more it will be probable. It preserves the shrinking capabilities of the underlying arbitrary
