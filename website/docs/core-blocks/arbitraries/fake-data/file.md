...
- `maxLength?` — default: `0x7fffffff` [more](/docs/tips/larger-entries-by-default#size-explained) — _maximal number of characters (included if multiple of 4)_
...
- `depthSize?` — default: `undefined` [more](/docs/tips/larger-entries-by-default#depth-size-explained) — _how much we allow our recursive structures to be deep?_
...
- `maxCount?` — default: `0x7fffffff` [more](/docs/tips/larger-entries-by-default#size-explained) — if `mode` is `"words"`: lorem ipsum sentence containing at most `maxCount` sentences, otherwise: containing at most `maxCount` words\_
