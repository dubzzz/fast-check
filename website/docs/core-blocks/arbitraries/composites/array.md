...
- `depthIdentifier?` — default: `undefined` — _when specified, the array will impact the depth attached to the identifier to avoid going too deep if it already generated lots of items for current level and will more likely generate entries having the minimal accepted length as the depth increases_
...
// Note: Thanks to the `depthIdentifier` it received, `fc.array` also tends to generate arrays having the minimal
// accepted length as we go deeper in the recursion, making it able to stop the recursion alone.
// Examples of generated values:
...
// • {__proto__:null,"value":15,"children":[{"value":1009275606,"children":[]},{"value":1086795967,"children":[]}]}
// • …
```
