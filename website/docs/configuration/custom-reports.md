...
const myCustomAssert = async (property, parameters) => {
  ...
  await throwIfFailed(runDetails);
};
```
...
const buildCodeSandboxReporter = (createFiles) => {
  return async function reporter(runDetails) {
    if (!runDetails.failed) {
      return;
    }
    const counterexample = runDetails.counterexample;
    const originalErrorMessage = await fc.defaultReportMessage(runDetails);
