...
No need to touch `call` itself. The function should always just simply return the inputs in receive. But instead of queueing it, we should queue a scheduled version of it. You may refer to [`scheduleFunction`](/docs/advanced/race-conditions/#schedulefunction) for more details.
...
Our test should think as a user of the API would think. It has to wait for all queued calls to be resolved before being able to assert of the output. You may want to use pass such condition to `waitFor` so that everything gets properly awaited before running any assertion.
