...
export const queueCodeV1 = `export function queue(fun) {
...
export const queueCodeV2 = `export function queue(fun) {
    let pastQueries = [];
    return (...args) => {
      const currentQuery = fun(...args);
...
export const queueCodeV3 = `export function queue(fun) {
    let pastQueries = [];
    return (...args) => {
      const currentQuery = fun(...args);
...
export const queueCodeV4 = `export function queue(fun) {
    let queryId = 0;
    let lastQuery = null;
    return (...args) => {
...
export const queueCodeV5 = `export function queue(fun) {
    let pending = false;
    let onDone = [];
    function runNext() {
      if (onDone.length === 0) {
    ...
    return (...args) => {
      if (!pending) {
        ...
        return new Promise((resolve, reject) => {
          const p = fun(...args);
          p.then(v => { runNext(); return v; }, e => { runNext(); throw e; })
           .then(resolve, reject);
        });
      }
      return new Promise((resolve, reject) => {
        onDone.push(() => {
          const p = fun(...args);
          p.then(v => { runNext(); resolve(v); }, e => { runNext(); reject(e); });
        });
      });
    };
  }`;

export const queueUnitSpecCode = `import {queue} from './queue.js';
...
export const queueBasicPBTSpecCode = `import {queue} from './queue.js';
...
export const queueBasicPBTWaitAllSpecCode = `import {queue} from './queue.js';
  import fc from 'fast-check';
...
export const queueMoreThan2CallsPBTSpecCode = `import {queue} from './queue.js';
  import fc from 'fast-check';
...
export const queueBatchesAlternativePBTSpecCode = `import {queue} from './queue.js';
  import fc from 'fast-check';
...
export const queueFromBatchesPBTSpecCode = `import {queue} from './queue.js';
  import fc from 'fast-check';
...
export const missingPartPBTSpecCode = `import {queue} from './queue.js';
  import fc from 'fast-check';
...
export const extendedBackToWaitAllPBTSpecCode = `import {queue} from './queue.js';
  import fc from 'fast-check';
...
export const extendedWithExceptionsPBTSpecCode = `import {queue} from './queue.js';
  import fc from "fast-check";
