...
An alternative would have been to use `waitAll` instead of `waitFor` but it comes with a precise requirement: promises have to be imperatively scheduled by the time we reach them. In other words, if our code delays a little bit the call to `call`, the scheduler might not wait enough.
