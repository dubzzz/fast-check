...
async function collectAsset(assetName, resultingFilePath) {
...
async function syncStaticAssets() {
  ...
  const pendingScans = (
    await glob(`./{blog,docs,src}/**/*.{js,ts,jsx,tsx,md}`, {
      withFileTypes: true,
      nodir: true,
    })
    ...
    for (const m of fileContent.matchAll(imageMatcherRegex)) {
      if (!pendingImages.has(m[1])) {
        ...
        if (existsSync(resultingFilePath)) {
          ...
          pendingImages.set(m[1], collectAsset(m[1], resultingFilePath));
        }
      }
    }
  });
