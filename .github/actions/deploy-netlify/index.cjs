...
const execAsync = (command, options) => {
  const prettyCmd = `exec(${JSON.stringify(command)}, ${JSON.stringify(
    options
  )}})`;
...
async function run() {
  ...
  if (err && err.code) {
    core.setFailed(
      `deploy-netlify failed to get back commit hash, failed with error: ${err}`
    );
    return;
  }
  ...
  if (!netlifyUrlLine) {
    core.setFailed(
      `deploy-netlify failed to find the deployment line in:\n\n${netlifyLog}`
    );
    return;
  }
  ...
  if (!m) {
    core.setFailed(
      `deploy-netlify failed to find the deployment url in:\n${netlifyUrlLine}`
    );
    return;
  }
  ...
  const body =
    `Give a try to https://github.com/dubzzz/fast-check/pull/${
      context.issue.number
    }/commits/${commitHash.split("\n")[0].trim()} with:\n\n` +
    "```bash\n" +
    `yarn add ${packageUrl}\n` +
    `npm i ${packageUrl}\n` +
    "```\n\n" +
